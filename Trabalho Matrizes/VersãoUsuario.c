...
Matriz* create_matrix(int rows, int cols, float values[][cols]) {
...
int main(void) {
    ...
    Matriz* A = create_matrix(4, 4, a_values);
    ...
    Matriz* B = create_matrix(4, 4, b_values);
    ...
    Matriz* D = matrix_multiply(A, B);
    printf("Matriz Multiplicação:\n");
    matrix_print(D);
    matrix_destroy(D);

    // Transposição de Matriz A
    Matriz* E = matrix_transpose(A);
